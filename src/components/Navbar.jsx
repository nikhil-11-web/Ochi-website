...
import { IoMenu, IoClose } from "react-icons/io5"; // Install react-icons if needed
...
const Navbar = () => {
  ...
  return (
    <>
      <div className='fixed z-[999] w-full px-5 md:px-20 py-4 md:py-8 font-["Neue_Montreal"] flex justify-between items-center backdrop-blur-md bg-zinc-900/50 md:bg-transparent'>
        
        {/* Logo */}
        <div className='logo cursor-pointer z-[1000]'>
          <svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white">
        ...
        <div className='links hidden md:flex gap-10'>
          {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
            <a key={index} className={`text-md capitalize font-light hover:text-gray-400 transition-colors cursor-pointer ${index === 4 && "ml-32"}`}>
              {item}
            </a>
          ))}
        </div>
        ...
        <div className='md:hidden z-[1000] cursor-pointer' onClick={toggleMenu}>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className='fixed top-0 left-0 w-full h-screen bg-zinc-900 z-[998] flex flex-col items-center justify-center gap-10 font-["Founders_Grotesk"]'
          >
            {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
              <a 
                key={index} 
                className='text-5xl uppercase font-bold text-white hover:text-[#CDEA68] transition-colors cursor-pointer'
                onClick={toggleMenu}
              >
                {item}
              </a>
